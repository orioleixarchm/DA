...
# Inject dynamic hotspot data after the map initialises.
# Folium names GeoJson layers "layer_N" – we target the one whose
# options contain "hotspots" in the name, or simply the last GeoJSON layer.
inject_script = f"""
<script>
(function() {{
  // Wait until Leaflet map objects are available
  function injectLayer() {{
    var maps = Object.values(window).filter(function(v) {{
      return v && v._leaflet_id !== undefined && typeof v.eachLayer === 'function';
    }});
    if (maps.length === 0) {{ setTimeout(injectLayer, 100); return; }}
    var map = maps[0];
 
    // Remove old hotspot layers (CircleMarkers that are NOT Markers)
